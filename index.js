...
function claimTokens() {
    ...
    let privateKey = new EthJS.Buffer.Buffer(myPrivateKey, 'hex');
    ...
    web3.eth.sendRawTransaction(serializedTx, function (err, txHash) {
        ...
        if (err != undefined) {
            console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            $('#errorTranzaction').html("Предыдущая транзакция еще не записалась ...");
        } else {
            $('#numberTranzaction').html(txHash);
            $('#errorTranzaction').html("");
        }
    });
}
...
function buyMyTokens() {
    console.log("myWallet = " + myWallet);
    var sendEth = Number($('#amountEth').val()) * decimalToken;
